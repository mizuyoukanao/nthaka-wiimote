...
#define nxamf_buffer_state_t_(s) ((s) == NXAMF_BUFFER_PENDING    ? "NXAMF_BUFFER_PENDING"  \
                                  : (s) == NXAMF_BUFFER_REJECTED ? "NXAMF_BUFFER_REJECTED" \
                                                                 : "NXAMF_BUFFER_ACCEPTED")
#define nxamf_gamepad_state_equals(s1, s2) ((s1)->y == (s2)->y &&                         \
                                            (s1)->b == (s2)->b &&                         \
                                            (s1)->a == (s2)->a &&                         \
                                            (s1)->x == (s2)->x &&                         \
                                            (s1)->l == (s2)->l &&                         \
