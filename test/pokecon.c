...
static int test_update(void)
{
    ...
    for (size_t i = 0; i < SIZE_OF(cases); i++)
    {
        ...
        nthaka_format_t *fmt = (nthaka_format_t *)&pokecon;
...
static int test_deserialize(void)
{
    ...
    test_case_t cases[] = {
        ...
        {.buf = "0x0004 8\r\n", .size = 10, .expected = {.y = NTHAKA_BUTTON_STATE_PRESSED, //
...
static int test_hold_previous_state(void)
{
    ...
    test_case_t cases[] = {{.prev = "0x0003 8 80 ff ff 80\r\n", //
                            ...
                            .buf = "0x0004 8\r\n",
                            .size = 10,
                            .expected = {.y = NTHAKA_BUTTON_STATE_PRESSED, //
                                         .b = NTHAKA_BUTTON_STATE_RELEASED,
                                         .a = NTHAKA_BUTTON_STATE_RELEASED,
                                         .x = NTHAKA_BUTTON_STATE_RELEASED,
                                         .l = NTHAKA_BUTTON_STATE_RELEASED,
...
static int test_reset(void)
{
    ...
    assert(nthaka_format_update(fmt, 'x') == NTHAKA_BUFFER_PENDING);
    assert(nthaka_format_update(fmt, '0') == NTHAKA_BUFFER_PENDING);
    assert(nthaka_format_update(fmt, '0') == NTHAKA_BUFFER_PENDING);
    assert(nthaka_format_update(fmt, '0') == NTHAKA_BUFFER_PENDING);
    assert(nthaka_format_update(fmt, '0') == NTHAKA_BUFFER_PENDING);
    assert(nthaka_format_update(fmt, ' ') == NTHAKA_BUFFER_PENDING);
    assert(nthaka_format_update(fmt, '8') == NTHAKA_BUFFER_PENDING);
    assert(nthaka_format_update(fmt, '\r') == NTHAKA_BUFFER_PENDING);
    assert(nthaka_format_update(fmt, '\n') == NTHAKA_BUFFER_ACCEPTED);
